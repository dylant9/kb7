...
static void make_slot(uint8_t *slot, uint32_t generation, uint8_t fill) {
    struct kb7_slot_header header;
    uint8_t payload[48];
    memset(slot, 0xff, KB7_STORAGE_SCREEN_SLOT_BYTES);
...
int main(void) {
    ...
    make_slot(slot_a, 1U, 0x11U);
    make_slot(slot_b, 2U, 0x22U);
    ...
    if (kb7_crc32_finish(crc) != kb7_crc32(sample, sizeof(sample))) return 4;
    return 0;
}
